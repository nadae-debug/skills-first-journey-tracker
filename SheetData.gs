...
/** Reads the whole Master Spreadsheet tab, dedupes by canonical id (keeping
 * the richest row per id, exactly like the old client-side sync did), and
 * merges in curated overrides (previousAssessments/engagementIdeas/etc). */
function readCompanies_() {
  ...
  table.rows.forEach(function (r) {
    ...
    if (!existing || richness_(cand) >= richness_(existing)) byId[id] = cand;
  });
...
/** Finds the sheet row for a company id by recomputing canonical ids from
 * the Company name column (same logic as readCompanies_), so it stays
 * correct even if rows get reordered between calls. Returns the row number
 * of the richest match, or 0 if the company has no row yet. */
function findRowForId_(sh, companyId) {
  ...
  table.rows.forEach(function (r) {
    var rawName = get_(r, COL_COMPANY_NAME);
    if (!rawName) return;
    if (slugify_(canonicalName_(rawName)) !== companyId) return;
    var score = r.__row; // prefer the last (most recently edited) matching row
    if (score > bestScore) { bestScore = score; best = r.__row; }
  });
  return best;
}
...
function deleteCompany_(companyId) {
  var sh = sheet_(TAB_MASTER);
  var row = findRowForId_(sh, companyId);
